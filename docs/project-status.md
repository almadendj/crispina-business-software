...
1. ~~**CRI-1** — scaffold the Next.js PWA.~~ **Done** — Next.js 16 (App Router,
   TS, Tailwind v4) + Serwist (Workbox) PWA: manifest, auto-registered service
   worker precaching the app shell, installable; Prettier + ESLint + `.env`
   handling. Build/lint/type-check all pass. Note: build runs on the **webpack**
   engine (`next build --webpack`) because Serwist's stable plugin needs webpack,
   not Next 16's default Turbopack.
